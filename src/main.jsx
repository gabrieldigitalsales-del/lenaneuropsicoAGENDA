...
function whatsappUrl(phone, appointment) {
  const digits = normalizePhone(phone)
  if (!digits) return null
  const br = digits.startsWith('55') ? digits : `55${digits}`
  const patientName = appointment?.patient_name || appointment?.patient_name_snapshot || 'paciente'
...
function AgendaApp({ onLogout }) {
  ...
  async function updateRecurringGroup(item, updateData, scope) {
    ...
    const updates = targets.map(t => ({ ...t, ...updateData, id: t.id, patient_name_snapshot: updateData.patient_name || t.patient_name, patient_phone_snapshot: updateData.patient_phone || t.patient_phone }))
    if (hasSupabase) {
  ...
  const nextAppointment = todayAppointments.find(a => String(a.appointment_time).slice(0,5) >= new Date().toTimeString().slice(0,5)) || todayAppointments[0]
  ...
  return <div className="phone-app">
    ...
    {modal?.type === 'appointment' && <AppointmentModal item={modal.item} patients={patients} selectedDate={selectedDate} settings={settings} onClose={()=>setModal(null)} onSave={saveAppointment} onUpdateGroup={updateRecurringGroup} onCreatePatient={()=>setModal({type:'patient'})} />}
  </div>
}
...
function AppointmentCard({a,onEdit,onDelete}) {
  const w = whatsappUrl(a.patient_phone, a)
...
function PatientCard({p, appointments, onEdit, onDelete}) {
  ...
  const w = whatsappUrl(p.phone || p.whatsapp)
...
function AppointmentModal({item,patients,selectedDate,settings,onClose,onSave,onUpdateGroup,onCreatePatient}) {
